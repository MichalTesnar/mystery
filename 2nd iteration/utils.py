...
def retrain_model(name, current_model, current_x_train, current_y_train):
    if name == "Ensembles":
        return retrain_ensembles(current_model, current_x_train, current_y_train)
    if name == "Dropout":
        return retrain_dropout(current_model, current_x_train, current_y_train)
    raise Exception("UTILS: Invalid model name / model not implemented.")
...
def retrain_dropout(model, x_train, y_train):
    # training the model
    model.fit(x_train, y_train, verbose=False, epochs=EPOCHS)
    return model
